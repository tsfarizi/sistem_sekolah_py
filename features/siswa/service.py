...
from features.siswa.repository import (
    get_all_siswa as repo_get_all,
    get_siswa_by_kelas,
    get_siswa_by_nis as repo_get_by_nis,
...
def detail_siswa(db: Session, nis: str) -> Siswa:
    siswa = repo_get_by_nis(db, nis)
    if not siswa:
        raise NotFoundException("Siswa tidak ditemukan")
    return siswa


def create_siswa(db: Session, data: SiswaCreate) -> Siswa:
    nis = data.nis if data.nis else _generate_nis(db)
    existing = repo_get_by_nis(db, nis)
    if existing:
...
def update_siswa(db: Session, nis: str, data: SiswaUpdate) -> Siswa:
    siswa = repo_get_by_nis(db, nis)
    if not siswa:
        raise NotFoundException("Siswa tidak ditemukan")
...
def delete_siswa(db: Session, nis: str) -> None:
    siswa = repo_get_by_nis(db, nis)
    if not siswa:
        raise NotFoundException("Siswa tidak ditemukan")
